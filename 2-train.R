...
COLUMNS = c(
  ...
  "name_family_status",
  "name_housing_type",
  "region_population_relative", "days_birth", "days_employed", "days_registration", "days_id_publish", "flag_mobil", "flag_emp_phone", "flag_work_phone", "flag_cont_mobile", "flag_phone", "flag_email",
  "occupation_type",
...
fit_glm <- train(as.factor(target) ~ ., data = train, method = "glm",
                 metric = "ROC",
...
fit_rpart <- train(as.factor(target) ~ ., data = train, method = "rpart",
                   metric = "ROC",
                   trControl = trainControl(
                     method = "cv",
                     number = 10,
